...
const handleWsConnection = async function (socket, theStorage, bacnetProcess) {
  webSock = socket;
...
const storeAndTransferData = function (options) {
  ...
  const handleToWSS = socket => handleWsConnection(socket, theStorage, bacnetProcess);
  ...
  bacnetProcess.on('message', message => {
    if (message.cmd === 'gotCOV') {
      ...
      if ((nr !== -1) && (webSock !== undefined)) {
        if (switches[nr].state !== message.value.value) {
          switches[nr].state = message.value.value;
          const changed = [{ id: switches[nr].id, name: switches[nr].name, state: switches[nr].state }];

          debug('handleWsConnection::changes.switch = ', changed);
          webSock.send(JSON.stringify({ type: 'changes', value: changed }));
        }
      }
    }
    ...
    if (message.cmd === 'gotSched') {
      ...
      if (nr !== -1) {
        ...
        webSock.send(JSON.stringify({ type: 'readSchedule', value: { id: switches[nr].id, name: switches[nr].name, val: switches[nr].weeklySchedule }}));
      }
    }
