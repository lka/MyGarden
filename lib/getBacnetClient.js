...
const getBacnetClient = function () {
  ...
  client.on('iAm', device => {
    let binaryOutputsList = [];
    const binaryOutputArray = [];
    ...
    client.readPropertyMultiple(device.address, requestDeviceObjectMultiple, (err, value) => {
      ...
      binaryOutputsList = binaryOutputs.map(x => x.value);
    ...
    binaryOutputsList.forEach(xxx => {
      const requestBinaryOutputMultiple = [{
        objectIdentifier: xxx,
        propertyReferences: [{ propertyIdentifier: 8 }]
      }];

      client.readPropertyMultiple(device.address, requestBinaryOutputMultiple, (error, val) => {
        if (error) {
          throw new Error(error, error.message);
        }
        const objectName = val.values[0].values.find(y => y.propertyIdentifier === 77);

        binaryOutputArray.push({
          objectIdentifier: val.values[0].objectIdentifier,
          objectName: objectName.value[0].value
        });
        /* eslint-disable no-console */
    ...
    const readDevice = {
      ...
      binaryOutputs: binaryOutputArray
    };

    devices.push(readDevice);
    /* eslint-disable no-console */
