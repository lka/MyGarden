...
const storage = function (options) {
  ...
  api.get('/changes', (req, res) => {
    if (switches.length) {
      res.send(switches.filter(val => val.sticky === true));
      switches = switches.map(val => ({ id: val.id, name: val.name, state: val.state, sticky: false }));
    }
  });
  ...
  baclient.on('gotDevice', () => {
    ...
    baclient.devices.forEach(z => {
      ...
      z.binaryOutputs.forEach(az => {
        ...
        if (switches.findIndex(xx => xx.name === az.objectName) === -1) {
          switches.push({ device: z.deviceID, id: uuid(), name: az.objectName });
          sticky = true;
        }
      });
    });
    if (sticky) {
      ...
      arr.push(JSON.stringify(switches));
      theStorage.put('binary', streamify(arr, os.EOL), error => {
