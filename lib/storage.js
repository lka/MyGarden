...
const writeToStorage = function (storage) {
  ...
  passThrough.write(JSON.stringify(switches.map(z => ({ id: z.id, deviceId: z.deviceId, objectType: z.objectType, objectId: z.objectId, name: z.name, showItem: z.showItem }))));
  passThrough.end();
};
...
const storage = function (options) {
  ...
  theStorage.get('binaries', (err, stream) => {
    ...
    if (err) {
      ...
      stream.pipe(concat(data => {
        ...
        switches = theArr.map(val => ({ id: val.id, deviceId: val.deviceId, objectType: val.objectType, objectId: val.objectId, name: val.name, showItem: val.showItem, state: DefaultState, sticky: true }));
      }));
    }
  });
  ...
  api.get('/schedule', (req, res) => {
    ...
    debug('get /schedule::switches.length = ', switches.length, id);
    if ((switches.length <= 0) || (i === -1)) {
      return res.sendStatus(404);
    }
    res.send(JSON.stringify(switches.filter(x => x.id === id).map(z => ({ id: z.id, name: z.name, val: z.weeklySchedule }))));
  ...
  api.get('/objects', (req, res) => {
    debug('get /objects::switches.length = ', switches.length);
    if (switches.length <= 0) {
      return res.sendStatus(404);
    }
    res.send(JSON.stringify(switches.map(z => ({ id: z.id, name: z.name, val: z.showItem }))));
  });
  ...
  api.get('/binaries', (req, res) => {
    ...
    res.send(JSON.stringify(switches.filter(val => val.showItem === true).map(z => ({ id: z.id, name: z.name, objectType: z.objectType }))));
    for (let i = 0; i < switches.length; i++) {
      if (switches[i].name === '') {
