...
const storage = function (options) {
  ...
  baclient.on('gotCOV', (devID, res) => {
    debug('got subscribedCOV', devID, res);
    const nr = switches.findIndex(xx => xx.deviceId === devID);

    if (nr !== -1) {
      if (switches[nr].state !== res.valueList[0].value) {
        switches[nr].state = res.valueList[0].value;
