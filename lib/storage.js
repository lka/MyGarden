...
const storage = function (options) {
  ...
  api.post('/language', (req, res) => {
    languageSet = req.body.language;
    debug('post language', languageSet);
  ...
  api.get('/binaries', (req, res) => {
    ...
    res.send(JSON.stringify({ language: languageSet, objects: switches.filter(val => val.showItem === true).map(z => ({ id: z.id, name: z.name, objectType: z.objectType })) }));
    for (let i = 0; i < switches.length; i++) {
      if (switches[i].name === '') {
