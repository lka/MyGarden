...
process.on('message', message => {
  ...
  if (message.cmd === 'writeWeeklySchedule') {
    debug('writeWeeklySchedule', message);
    BacnetClient.writeWeeklySchedule(baclient, message.value.deviceId, message.value.objectType, message.value.objectId, message.value.value, err => {
      if (err) {
        debug('Error: ', err);
      }
    });
  ...
  if (message.cmd === 'readSched') {
    debug('readSched');
    BacnetClient.readSched(baclient, message.value.deviceId, message.value.objectType, message.value.objectId, message.value.property, err => {
      if (err) {
        debug('Err:', err.message);
      }
    });
