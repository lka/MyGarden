...
class BacnetClient extends Bacstack {
  constructor (options) {
    ...
    this.on('covNotifyUnconfirmed', message => {
      debug(`BACnet 'covNotifyUnconfirmed' received, address: ${message.address}, result:`, message.request.values[0].value);
      const valueList = message.request.values;
      const presentValue = valueList.filter(x => x.property.id === 85);

      this.emit('gotCOV', { deviceId: message.request.initiatingDeviceId.instance, objectType: message.request.monitoredObjectId.type, objectId: message.request.monitoredObjectId.instance, value: presentValue[0].value[0].value });
    });
