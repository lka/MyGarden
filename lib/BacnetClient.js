...
class BacnetClient extends Bacstack {
  ...
  static cancelObservation (self, deviceId, objectType, objectInstance, index, next) {
    ...
    self.subscribeCOV(self.devices[nr].address, { type: objectType, instance: objectInstance }, baEnum.BacnetPropertyIds.PROP_PRESENT_VALUE, true, false, self.COVTIMEOUT, err => {
      if (err) {
  ...
  static readSched (self, deviceId, objectType, objectInstance, index, next) {
    ...
    BacnetClient.read(self, deviceId, objectType, objectInstance, baEnum.BacnetPropertyIds.PROP_WEEKLY_SCHEDULE, (innerErr, val) => {
      if (innerErr) {
        return next(null);
      }
      debug('gotSched to emit', val.valueList);
      self.emit('gotSched', { deviceId, objectType: val.objectId.type, objectId: val.objectId.instance, value: val.valueList });
    });
    next(null);
  }
}
