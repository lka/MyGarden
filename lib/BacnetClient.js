...
class BacnetClient extends Bacstack {
  ...
  static writeBinaryOutput (self, deviceId, objectInstance, value, next) {
    ...
    debug(`writeBinaryOutput(address: ${self.devices[nr].address} ObjectInstance: ${objectInstance}, value: ${value})`);
    self.writeProperty(self.devices[nr].address, baEnum.BacnetObjectTypes.OBJECT_BINARY_OUTPUT,
  ...
  static readBinaryOutputValue (self, deviceId, objectInstance, next) {
    const nr = self.devices.findIndex(x => x.deviceId === deviceId);

    debug(`readBinaryOutputValue(address: ${self.devices[nr].address} ObjectInstance: ${objectInstance})`);
    self.readProperty(self.devices[nr].address, baEnum.BacnetObjectTypes.OBJECT_BINARY_OUTPUT, objectInstance, baEnum.BacnetPropertyIds.PROP_PRESENT_VALUE, null, (error, val) => {
      if (error) {
        return next(error);
      }
      next(null, val);
  ...
  static subscribeCOVBinaryOutput (self, deviceId, objectInstance, index, next) {
    const nr = self.devices.findIndex(x => x.deviceId === deviceId);
    ...
    debug(`subscribeCOVBinaryOutput(address: ${self.devices[nr].address} ObjectInstance: ${objectInstance})`);
    self.subscribeCOV(self.devices[nr].address, { type: baEnum.BacnetObjectTypes.OBJECT_BINARY_OUTPUT,
      instance: objectInstance }, baEnum.BacnetPropertyIds.PROP_PRESENT_VALUE, false, false, self.COVTIMEOUT, err => {
      if (err) {
        // return next(err);
        self.timers.push(setInterval(() => {
          BacnetClient.readBinaryOutputValue(self, deviceId, objectInstance, (innerErr, val) => {
            if (innerErr) {
              // throw new Error(innerErr);
              return next(null);
            }
            debug('gotValue to emit', val.valueList[0].value);
            self.emit('gotCOV', { deviceId, objectId: val.objectId.instance, value: val.valueList[0].value });
          });
        }, self.POLLINTERVAL));
      }
      next(null);
