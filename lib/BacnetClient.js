...
class BacnetClient extends Bacstack {
  constructor (options) {
    ...
    this.devices = [{ address: '192.168.31.1', deviceID: 99, binaryOutputs: [{ objectIdentifier: { type: 4, instance: 0 }, objectName: 'irgendwas' }]}];
      ...
      binaryOutputs.map(x => x.value).forEach(xxx => {
        self.readProperty(device.address, xxx.type, xxx.instance, 77, null, (error, val) => {
          ...
          if (count === 0) {
            ...
            debug(`CountDevices: ${this.devices.length}`);

            self.devices.forEach(z => {
              debug(`DeviceAddress: ${z.address}, deviceId: ${z.deviceId}`);
              z.binaryOutputs.forEach(az => {
                debug(`deviceId: ${z.deviceId}:BinaryObject: ${az.objectIdentifier.instance}, objectName: ${az.objectName}`);
              });
            });
          }
        });
  ...
  static writeBinaryOutputValue (self, address, objectInstance, value, next) {
    ...
    self.writeProperty(address, 4, objectInstance, 85, self.priorityForWriting, [{ type: 9, value }], err => {
      if (err) {
        return next(err);
