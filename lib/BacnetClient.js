...
class BacnetClient extends Bacstack {
  ...
  static writeBinaryOutput (self, address, objectInstance, value, next) {
    ...
    debug(`writeBinaryOutput(address: ${address} ObjectInstance: ${objectInstance}, value: ${value})`);
  ...
  static readBinaryOutputValue (self, address, objectInstance, next) {
    debug(`readBinaryOutputValue(address: ${address} ObjectInstance: ${objectInstance})`);
    self.readProperty(address, baEnum.BacnetObjectTypes.OBJECT_BINARY_OUTPUT, objectInstance, baEnum.BacnetPropertyIds.PROP_PRESENT_VALUE, null, (error, val) => {
      if (error) {
        return next(error);
      }
      next(null, val);
  ...
  static subscribeCOVBinaryOutput (self, address, objectInstance, index, next) {
    const nr = self.devices.findIndex(x => x.address === address);
    const TIMEOUT = 3600;
    const INTERVAL = 2000;
    ...
    debug(`subscribeCOVBinaryOutput(address: ${address} ObjectInstance: ${objectInstance})`);
    self.subscribeCOV(address, { type: baEnum.BacnetObjectTypes.OBJECT_BINARY_OUTPUT,
      instance: objectInstance }, baEnum.BacnetPropertyIds.PROP_PRESENT_VALUE, false, false, TIMEOUT, err => {
      if (err) {
        // return next(err);
        self.timers.push(setInterval(() => {
          self.readBinaryOutputValue(self, address, objectInstance, (innerErr, val) => {
            if (innerErr) {
              throw new Error(innerErr);
            }
            self.emit('subscribeCOV', val);
          });
        }, INTERVAL));
      }
      next(null);
