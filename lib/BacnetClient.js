...
class BacnetClient extends Bacstack {
  ...
  static writeBinaryOutputValue (self, address, objectInstance, value) {
    ...
    const theValue = value === null ? { type: 0 } : { type: 1, value };

    self.writeProperty(address, 4, objectInstance, 87, self.priorityForWriting, [ theValue ], (err, val) => {
      if (err) {
        throw new Error('writing failed', err);
      }
